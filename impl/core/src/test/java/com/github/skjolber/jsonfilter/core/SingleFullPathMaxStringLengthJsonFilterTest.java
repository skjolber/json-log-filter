...
public class SingleFullPathMaxStringLengthJsonFilterTest extends DefaultJsonFilterTest {
	...
	public void exception_returns_false() throws Exception {
		assertFalse(new SingleFullPathMaxStringLengthJsonFilter(-1, -1, PASSTHROUGH_XPATH, FilterType.ANON).process(new char[] {}, 1, 1, new StringBuilder()));
	}

	@Test
	public void exception_offset_if_not_exceeded() throws Exception {
		assertNull(new SingleFullPathMaxStringLengthJsonFilter(-1, -1, PASSTHROUGH_XPATH, FilterType.ANON).process(TRUNCATED));
		assertFalse(new SingleFullPathMaxStringLengthJsonFilter(-1, -1, PASSTHROUGH_XPATH, FilterType.ANON).process(FULL, 0, FULL.length - 3, new StringBuilder()));
	}
