...
public class JacksonAnyPathMaxStringLengthJsonFilter extends AbstractMultiPathJsonFilter implements JacksonJsonFilter {

	protected final JsonFactory jsonFactory;
	private final Map<String, FilterType> fields;
	
	public JacksonAnyPathMaxStringLengthJsonFilter(int maxStringLength, String[] anonymizes, String[] prunes) {
		this(maxStringLength, anonymizes, prunes, new JsonFactory());
	}

	public JacksonAnyPathMaxStringLengthJsonFilter(int maxStringLength, String[] anonymizes, String[] prunes, JsonFactory jsonFactory) {
	...
	public JacksonAnyPathMaxStringLengthJsonFilter(int maxStringLength, String[] anonymizes, String[] prunes, String pruneMessage, String anonymizeMessage, String truncateMessage) {
		this(maxStringLength, anonymizes, prunes, pruneMessage, anonymizeMessage, truncateMessage, new JsonFactory());
	}

	public JacksonAnyPathMaxStringLengthJsonFilter(int maxStringLength, String[] anonymizes, String[] prunes, String pruneMessage, String anonymizeMessage, String truncateMessage, JsonFactory jsonFactory) {
		...
		if(prunes != null) {
			for(int i = 0; i < prunes.length; i++) {
				...
				String name = removeAnyPrefix(prunes[i]);
				fields.put(name, FilterType.PRUNE);
			}
		}
		...
		if(anonymizes != null) {
			for(int i = 0; i < anonymizes.length; i++) {
				...
				String name = removeAnyPrefix(anonymizes[i]);
				fields.put(name, FilterType.ANON);
			}
		}
	}
