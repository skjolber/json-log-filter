...
/**
 * Shopify order array benchmark files generator.
 *
 * Input: shopify_orders.json.gz (compact JSON with 200 Shopify order objects)
 * Output: Sized files truncated at complete order boundaries.
 *
 * Structure: {"orders": [{order_obj}]}
 * root=1, order_obj=2, nested=3+. Cut at level==1 after each complete order closes.
 */
public class ShopifyFilesGenerator {

    private final File source;
    private final JsonFactory jsonFactory;

    public ShopifyFilesGenerator(File source) {
        this.source = source;
            ...
            try (JsonParser parser = jsonFactory.createParser(gzis);
                ...
                do {
                    ...
                    if (nextToken == JsonToken.START_OBJECT) {
                        ...
                        level--;
                        // Cut after each complete top-level order object.
                        // Orders are at depth 2 (root=1, order=2),
                        // so level==1 after END_OBJECT means an order just completed.
                        if (level == 1) {
                            if (bout.size() >= size) {
                                break;
                            }
    ...
    public static void main(String[] args) throws IOException {
        File file = new File("src/test/resources/benchmark/shopify_orders.json.gz");
        ShopifyFilesGenerator generator = new ShopifyFilesGenerator(file);
        generator.generateFileSizes(
            new File(file.getParent(), "shopify"),
